...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(
        "Sorry, there was a problem with the analysis. Please try again."
      );
      setAppState(AppState.INPUT);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
        ...
        <div className="w-full max-w-screen-2xl mx-auto px-4 md:px-6 h-20 md:h-24 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            >
              ...
              <span className="font-bold hidden sm:inline">History</span>
              <span className="bg-slate-300 text-slate-700 text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">
                {history.length}
              </span>
            </button>
          </div>
        ...
        {appState === AppState.ANALYZING && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[60vh] animate-fade-in">
            ...
            <p className="text-slate-500 max-w-xl text-lg md:text-xl leading-relaxed">
              Examining every aspect of your business model to identify
              strengths and weaknesses. Please wait a moment.
            </p>
          </div>
        )}
      ...
      <footer className="bg-white border-t border-slate-200 py-8 md:py-10 text-center text-slate-400 text-sm md:text-base">
        <p>
          © {new Date().getFullYear()} BMC Analyst. Powered by Nazmul H Roman.
        </p>
      </footer>
    </div>
  );
